---
description: '实施 tasks（按 routing.implementer 渲染：claude=当前会话本人实施 / 外部后端=委托 ly-wrapper 单次 agentic 调用）；全部任务完成后立即 commit apply: <change-name>；失败原样呈报转人工（不重试不兜底）'
---
