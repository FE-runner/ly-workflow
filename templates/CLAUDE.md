...
| `propose.md` | 真逻辑 | 委托 `opsx:propose` + 创建方案前 worktree 询问（从当前分支切出，不在 worktree 内才问，全局一次）+ 全自动/手动两路径（全自动 = review-plan → apply → review-code 自动化流水线；手动 = 逐步确认）；生成后 commit 前执行**方案自审**（正向/反向逻辑闭环 + 基线波及 + 通用业务维度过网，逐项结论清单硬约束；机械断链直接修、业务判断类 AskUserQuestion 问用户——全自动模式下仍问）；自审修复随 `propose: <change-name>` commit 一次干净落库 |
