...
**Goals:**
- 第 2 轮起，TASK 只传"上一轮 Critical 原文 + 本轮改动文件路径"，让 Codex 自行读取判断，降低每轮重传的内容量。
...
**2. 第 2 轮起：TASK 只含"上一轮 Critical 原文 + 改动文件路径"，不含未改动内容**

第 2 轮起的审查目的从"发现问题"变成"验证修复是否到位 + 检测修复是否引入新问题"。验证修复只需要 Codex 对照"上一轮它自己报告的 Critical 原文"和"该 Critical 所指文件的当前内容"；检测新问题只需要 Codex 读取"本轮实际被改动过的文件"。未被本轮触及的文件，其内容相对上一轮未变，重复传入不会带来新发现——Codex 面对相同输入大概率给出相同结论，重传的边际价值低，边际成本（token/时间）不低。
