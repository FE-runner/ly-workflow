...
- [ ] 4.1 版本号处理（前置条件：当前仓库 1.8.0 已 bump 但 **尚未发布**——npm latest / git tag 均为 1.7.5）：发版时若 v1.8.0 尚未单独发布，本 change 合并进 1.8.0 条目、不再 bump；若 1.8.0 已发布则三处同步 bump → 1.9.0（`package.json` / `codeagent-wrapper/main.go` `version` / `src/utils/installer.ts` `EXPECTED_BINARY_VERSION`）——实施本任务时按当时发布状态二选一，SHALL NOT 跳号
- [ ] 4.2 文档同步：根 `CLAUDE.md`（变更记录新条目 + 模块职责提 preflight）、`CHANGELOG.md` 新条目（版本号按 4.1 的判定）、`README.md`（若有安装前提/依赖描述则补一句"安装器会检测 openspec 依赖"）
