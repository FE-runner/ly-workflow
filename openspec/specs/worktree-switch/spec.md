...
- **WHEN** 用户执行 `/ly:worktree switch <change-name>`，`~/.ly/worktrees/<项目名>/<change-name>` 已是已注册的 worktree，但该路径当前注册的分支不是 `<change-name>`（例如被其他分支占用，或曾用于另一个 change 后未清理）
...
- **THEN** 系统基于 `git rev-parse --git-common-dir` 反推的主仓库位置计算目标路径，得到与在主工作区直接执行时完全相同的 `~/.ly/worktrees/<项目名>/<another-change-name>`，不会因为当前所处路径不同而算出错误或嵌套的路径
...
- **WHEN** 用户执行 `/ly:worktree switch <change-name>`，`~/.ly/worktrees/<项目名>/<change-name>` 路径已存在但不是 Git 已注册的 worktree
...
- **THEN** 系统输出形如 `cd ~/.ly/worktrees/<项目名>/<change-name> && claude "继续实施 change: <change-name>，读取 openspec/changes/<change-name>/tasks.md 按任务执行"` 的命令，且当前会话不做进一步动作
...
- **THEN** 系统输出形如 `cd ~/.ly/worktrees/<项目名>/<change-name> && claude "继续实施 change: <change-name>，读取 openspec/changes/<change-name>/tasks.md 按任务执行；实施完成后自动依次调用 /ly:review-code，按其全部终止条件运行，不需要人工确认"` 的命令，仍然只打印、不自动执行
