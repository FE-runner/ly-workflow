...
提供两个由 Codex 支撑的审查关卡——一个用于在实施前审查 OpenSpec change 的方案, 一个用于在实施后审查代码变更——统一使用 `ly-wrapper` TS 脚本（`--backend <init 选定的审查后端>`），替代旧的双模型（Codex + Gemini）交叉审查机制。两个命令都支持审查-修复循环：Codex 报告的 Critical 不是自动生效的裁决，Claude 先判断是否认可再决定是否修复，循环直到 Critical 清零或触发终止条件。
## Requirements
